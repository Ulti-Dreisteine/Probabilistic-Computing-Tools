...
def time2stp(t, time_format="%Y-%m-%dT%H:%M:%S+08:00"):
    """时间转为时间戳"""
    return int(time.mktime(time.strptime(t, time_format)) - time.mktime(t0))


def stp2time(stp, time_format="%Y-%m-%dT%H:%M:%S+08:00"):
    """时间戳转换为时间"""
    t_arr = time.localtime(stp + int(time.mktime(t0)))
    return time.strftime(time_format, t_arr)
...
def time2utc(t):
    ...
    return time.strftime("%Y-%m-%dT%H:%M:%S+08:00", time.localtime(stp))
...
def utc2stp(utc):
    ...
    return int(time.mktime(t_date) - time.mktime(t0))
...
def get_current_stp() -> float:
    return time.time() + time.mktime(t0)
