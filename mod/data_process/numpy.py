...
def compress_z_data(z_arr: np.ndarray) -> np.ndarray:
    ...
    if D == 1:
        ...
        i = 2
        z_compress = np.ravel_multi_index(z_arr[:i, :], z_label_ns[:i], mode="wrap") # type: ignore
        z_compress = _reorder_z_series(z_compress)

        while i != D:
            _arr = np.vstack((z_compress, z_arr[i, :]))
            z_compress = np.ravel_multi_index(_arr, (len(set(z_compress)), z_label_ns[i]), mode="wrap") # type: ignore
            z_compress = _reorder_z_series(z_compress)
            i += 1
