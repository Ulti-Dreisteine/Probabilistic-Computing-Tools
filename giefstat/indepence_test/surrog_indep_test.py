...
def exec_surrog_indep_test(x, y, method, z=None, xtype=None, ytype=None, ztype=None, rounds=100, 
    ...
    x: x数据, np.ndarray
    y: y数据, np.ndarray
    method: 选用的关联度量方法, str
    z: z数据, np.ndarray, defaults to None
    xtype: x的数值类型, str, defaults to None
    :ytype: y的数值类型, str, defaults to None
    :ztype: y的数值类型, str, defaults to None
    rounds: 重复轮数, defaults to 100
    alpha: 显著性阈值, defaults to 0.05
    max_size_bt: 用于自助重采样的最大样本数, defaults to 1000
    size_bt: 自行设定的重采样规模, int, defaults to None
    kwargs: cal_general_assoc方法中的关键字参数
